...
def run_VGGT(images, device, dtype, chunk_size):
    ...
    # 3. 加载本地权重
    local_weights_path = "/data/sazdata/VGGT-X/weights/model.pt"
    print(f"Loading weights from {local_weights_path}...")
    
    # 使用 weights_only=True 是好的做法，如果报错可尝试去掉
    state_dict = torch.load(local_weights_path, map_location="cpu", weights_only=True)
    
    # 如果权重是从原版模型修改而来，建议使用 strict=False 以防有微小差异
    model.load_state_dict(state_dict, strict=False)
    
    model.eval()
    model = model.to(device).to(dtype)
    
    # 重建任务通常不需要 track_head，手动置 None 减小开销
    model.track_head = None 
    print(f"Model loaded successfully")

    # 4. 执行推理逻辑
    with torch.no_grad():
        # VGGT 前向传播输出的是一个包含位姿编码、深度图等的字典
        predictions = model(images.to(device, dtype), verbose=True)
        
        # 将位姿编码转换为外参和内参矩阵
        # 注意 images.shape[-2:] 是原图的高宽，用于计算 FoV 还原内参
        extrinsic, intrinsic = pose_encoding_to_extri_intri(
            predictions['pose_enc'], 
            images.shape[-2:]
        )
        
        # 转换为 NumPy 格式供后续 COLMAP 流程使用
