...
def extract_matches(extrinsic, intrinsic, images, depth_conf, base_image_path_list, max_query_pts=4096, batch_size=128, err_range=20):
    ...
    # --- 修改开始：手动加载本地 XFeat ---
    xfeat_repo_path = "/data/sazdata/VGGT-X/accelerated_features-main/"
    if xfeat_repo_path not in sys.path:
        sys.path.append(xfeat_repo_path)
    
    # 从本地模块导入
    from modules.xfeat import XFeat
    
    # 1. 定义本地权重的绝对路径
    xfeat_weights_path = os.path.join(xfeat_repo_path, "weights", "xfeat.pt") 
    
    print(f"Loading XFeat model from local source with weights: {xfeat_weights_path}")
    
    # 2. 核心修改：在实例化时传入权重路径
    # 根据你提供的 XFeat.__init__ 定义，它会自动调用 self.net.load_state_dict
    xfeat = XFeat(weights=xfeat_weights_path, top_k=max_query_pts)
    
    # 3. 重要：务必确保删除了之前报错的那行 xfeat.load_state_dict(...)
    # 因为 XFeat 类内部已经把权重加载到了 self.net 中，
    # 外部再次对 xfeat 对象调用 load_state_dict 会因为缺少 "net." 前缀而报错。
    
    xfeat = xfeat.to(images.device).eval()
    # --- 修改结束 ---
